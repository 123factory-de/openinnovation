---
title: "100+ Accelerator"
date: 2026-06-09
draft: false
company: "AB InBev et al."
industry: "Sustainability"
externalUrl: "https://100accelerator.com/"
focusAreas:
  - "Water Stewardship"
  - "Circular Packaging"
  - "Climate Action"
  - "Sustainable Agriculture"
  - "Biodiversity"
eligibility:
  - "Startups"
  - "Scaleups"
status: "Active"
deadline: ""
summary: "100+ Accelerator는 글로벌 소비재 기업들이 함께 운영하는 지속가능성 중심 프로그램으로, 물, 패키징, 기후, 농업, 생물다양성 분야의 스타트업과 스케일업을 찾습니다."
---

## Overview

100+ Accelerator는 AB InBev, The Coca-Cola Company, Colgate-Palmolive, Danone, Mondelēz International, Unilever 등 주요 글로벌 기업이 함께 운영하는 오픈 이노베이션 프로그램입니다. 지속가능성 문제를 해결할 수 있는 스타트업과 스케일업을 대상으로 하며, 현재 활성 상태로 운영되고 있습니다.

## Focus Areas

- 물 관리
- 순환형 패키징
- 기후 대응
- 지속가능 농업
- 생물다양성

## Collaboration & Benefits

- 최대 10만 달러 규모의 파일럿 지원
- 멘토링과 실증 검증 지원
- 글로벌 공급망 및 대기업 파트너와의 협업 기회

## How to Apply

공식 100+ Accelerator 사이트에서 현재 운영 중인 프로그램 주기와 지원 절차를 확인한 뒤 신청합니다. 세부 일정과 제출 요건은 해당 기수 공고를 기준으로 다시 확인하는 것이 좋습니다.

## Sources

- [Official program page](https://100accelerator.com/)
